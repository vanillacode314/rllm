...
interface TAsyncResult<T, E> {
	...
	transpose(): Promise<T extends Option<infer U> ? Option<AsyncResult<U, E>> : never>;
	unwrap(): Promise<T>;
	unwrapErr(): Promise<E>;
...
class AsyncResult<T, E> implements TAsyncResult<T, E> {
	get catch() {
		return this.resultPromise.catch.bind(this.resultPromise);
	}
	get finally() {
	...
	private constructor(private readonly resultPromise: Promise<Result<T, E>>) {}
	...
	static fromThrowable<T, E>(
		f: () => MaybePromise<T>,
		onError: (e: unknown) => MaybePromise<E>
	): AsyncResult<T, E> {
		try {
			return new AsyncResult(
			...
			return new AsyncResult(Promise.resolve().then(async () => Result.Err(await onError(e))));
		}
	}
	...
	transpose(): Promise<T extends Option<infer U> ? Option<AsyncResult<U, E>> : never> {
		return this.resultPromise.then((r) => {
			if (r.isErr()) {
				return Option.Some(AsyncResult.fromResult(Result.Err(r.unwrapErr()))) as T extends (
					Option<infer U>
				) ?
					Option<AsyncResult<U, E>>
				:	never;
			} else {
				const value = r.unwrap();
				if (!Option.isOption(value)) {
					throw new Error(
						'called `AsyncResult.transpose()` on an `Ok` value that is not an `Option`'
					);
				}
				return value.map((u) => AsyncResult.fromResult(Result.Ok(u))) as T extends Option<infer U> ?
					Option<AsyncResult<U, E>>
				:	never;
			}
		});
