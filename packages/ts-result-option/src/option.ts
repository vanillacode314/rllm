...
class None<T = never> implements Option<T> {
	...
	transpose(): T extends Result<infer U, infer E> ? Result<Option<U>, E> : never {
		// As per Rust's std::Option, None.transpose() maps to Ok(None).
	...
	unzip(): T extends [infer A, infer B] ? [Option<A>, Option<B>] : never {
		return [
			this as Option<T extends [infer A, infer _] ? A : never>,
			this as Option<T extends [infer _, infer B] ? B : never>
...
class Option<T> {
	...
	static fromUndefinedOrNull<T>(value: null | T | undefined): Option<T> {
...
class Some<T> implements Option<T> {
	...
	transpose(): T extends Result<infer U, infer E> ? Result<Option<U>, E> : never {
		return (this.value as Result<any, any>).map(Option.Some) as T extends Result<infer U, infer E> ?
			Result<Option<U>, E>
		:	never;
	}
	transposePromise(): T extends Promise<infer U> ? Promise<Option<U>> : never {
		return (this.value as Promise<any>).then(Option.Some) as T extends Promise<infer U> ?
			Promise<Option<U>>
		:	never;
	}
	unwrap(): T {
	...
	unzip(): T extends [infer A, infer B] ? [Option<A>, Option<B>] : never {
		...
		return [
			Option.Some(this.value[0]) as Option<T extends [infer A, infer _] ? A : never>,
			Option.Some(this.value[1]) as Option<T extends [infer _, infer B] ? B : never>
		] as T extends [infer A, infer B] ? [Option<A>, Option<B>] : never;
	}
	xor(optb: Option<T>): Option<T> {
