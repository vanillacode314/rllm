...
export const rule = createRule({
  create(context) {
    ...
    const resultVariables = new Map<
      string,
      { node: any; returned: boolean; used: boolean; yielded: boolean }
    >();
    ...
    return {
      ...
      'Program:exit'() {
        for (const [, { node, returned, used, yielded }] of resultVariables.entries()) {
...
function isResultType(type: ts.Type, checker: ts.TypeChecker): boolean {
  const hasValidMethod = VALID_USAGE_METHODS.values().some((method) => type.getProperty(method));
