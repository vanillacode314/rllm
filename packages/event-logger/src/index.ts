	...
	for (const event of pending) {
		const update: TUpdate =
			event.operation === 'delete' ?
				{ id: event.id, operation: 'delete', table: event.table }
			:	{
					data: JSON.parse(event.data),
					id: event.id,
					operation: event.operation as 'insert' | 'update' | 'upsert',
					table: event.table
				};
		await tx.query(convertUpdateToStatement(update, event.timestamp, schema));
	}
