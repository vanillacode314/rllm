...
export type TConfig<TEvent extends TBaseEvent> = {
	...
	invalidate?: (items: Array<{ event: TEvent; keys: string[][] }>) => MaybePromise<void>;
...
export async function createEventLogger<TEvent extends TBaseEvent>({
	...
	const subscriptions = new Map<
		string,
		Set<{
			handler: (data: unknown, timestamp: string) => unknown;
			self?: boolean;
		}>
	>();
	...
	const logger = {
		...
		dispatch: async (
			...
			let updates: Array<{ event: TEvent; updates: TUpdate[] }> = [];
			const loggedEvents = await db.transaction(async (tx) => {
				...
				let loggedEvents: Array<{ data: string; timestamp: string; type: string }> | undefined;
				if (validatedEventsToLog.length > 0) {
					loggedEvents = await tx.query({
						...
						sql: `INSERT OR IGNORE INTO events (timestamp, type, data) VALUES ${validatedEventsToLog
							.map(() => '(?, ?, ?)')
							.join(',')} RETURNING timestamp, type, data`
					});
				}
				await logger.setClock(clock, tx);
				await logger.applyUpdates(
					updates.flatMap((update) => update.updates),
					clock.toString(),
					tx
				);
				return loggedEvents;
			...
			if (loggedEvents) {
				for (const { data, timestamp, type } of loggedEvents) {
					const subscribers = subscriptions.get(type);
					if (!subscribers) continue;
					for (const subscriber of subscribers) {
						if (!subscriber.self) continue;
						subscriber.handler(JSON.parse(data), timestamp);
					}
				}
			}
			await invalidate?.(
		...
		receive: async (
			...
			for (const { data, timestamp, type } of loggedEvents) {
				...
				if (!subscribers) continue;
				for (const subscriber of subscribers)
					tasks.push(Promise.resolve(subscriber.handler(JSON.parse(data), timestamp)));
			}
