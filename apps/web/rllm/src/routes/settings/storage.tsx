...
function SettingsStorageComponent() {
  ...
  const optimizeServerStorage = useMutation(() => ({
    mutationFn: () => {
      ...
      tryBlock(
        async function* () {
          const syncServerApi = yield* createAuthenticatedSyncServerFetcher();
...

          {
            ...
            while (true) {
              const { hasMore, messages, nextAfter } = yield* getServerMessages(
                $account.id,
                $account.aesKey,
                { after, clientId }
              );

              invalidate = await logger.receive(await optimizeMessages(messages));
              if (!hasMore) break;
          ...
          ws.addEventListener('message', async (e) => {
            const { timestamp } = safeParseJson(e.data, {
              validate: z.object({ timestamp: z.string() }).parse
            }).unwrap();
