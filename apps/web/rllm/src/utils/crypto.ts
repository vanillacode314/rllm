...
const encryptSingleChunk = (
  ...
  AsyncResult.from<Uint8Array, Error>(
    async function () {
...
const encryptMultiChunk = (
  ...
  AsyncResult.from(
    async function () {
      ...
      const encryptedChunkPromises: Promise<ArrayBuffer>[] = [];
      for (let i = 0; i < totalChunks; i++) {
        const start = i * MAX_CHUNK_SIZE;
        const end = Math.min(start + MAX_CHUNK_SIZE, data.byteLength);
        const chunk = data.slice(start, end);

        const encryptedChunkPromise = crypto.subtle.encrypt(
          {
            iv: iv,
            name: 'AES-GCM'
          },
  ...
  AsyncResult.from(
    async function () {
      const dataStart = decoder.decode(
        encryptedData.slice(0, Math.max(CHUNKED_TAG.byteLength, SINGLE_TAG.byteLength))
      );

      if (dataStart.startsWith(CHUNKED_TAG_STR)) {
        return decryptMultiChunk(encryptedData, aesKey, signal);
      } else if (dataStart.startsWith(SINGLE_TAG_STR)) {
        return decryptSingleChunk(encryptedData, aesKey, signal);
      } else {
        const taggedBuffer = new Uint8Array(SINGLE_TAG.byteLength + encryptedData.byteLength);
        taggedBuffer.set(SINGLE_TAG, 0);
        taggedBuffer.set(encryptedData, SINGLE_TAG.byteLength);
...
const decryptMultiChunk = (
  ...
  AsyncResult.from(
    async function () {
      ...
      const decryptedChunkPromises: Promise<ArrayBuffer>[] = [];
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        const decryptedChunkPromise = crypto.subtle.decrypt(
          {
            iv: iv,
            name: 'AES-GCM'
          },
      ...
      return Result.Ok(combinedBuffer);
    },
