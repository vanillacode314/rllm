								...
								async function () {
									...
									switch (payload.case) {
										case 'gotEvents': {
											...
											setNumberOfSendingChanges((value) => value - 1);
											break;
										}
										case 'info': {
										...
										case 'newEvents': {
											...
											const toInvalidate = await logger.receive(payload.value.id, decryptedEvents);
											await Promise.all(
	...
	tryBlock(
		async function* () {
			...
			ws.send(
				toBinary(
					SyncServerSocketRequestSchema,
					create(SyncServerSocketRequestSchema, { newEvents })
				)
			);
		},
