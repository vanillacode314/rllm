...
type TOpenAIChatCompletionRequest =
  typeof openaiChatCompletionRequestSchema.infer
...
const openaiChatCompletionResponseChunkDeltaSchema = type({
  ...
  'tool_calls?': asOption('unknown[]').pipe((value) =>
    value.map((tool_calls) =>
      tool_calls.map((value) => openaiToolCallResponseSchema.assert(value)),
    ),
  ),
}).pipe((value) =>
  Object.assign(
...
type TOpenAIChatCompletionResponseChunk =
  typeof openaiChatCompletionResponseChunkSchema.infer
