...
const openAiAdapter = {
	...
	handleChatCompletion: ({ chunks, onChunk, system, fetcher, model, signal, tools, onAbort }) =>
		tryBlock<void, Error>(
			async function* (): AsyncGenerator<Err<unknown, Error>, void> {
				...
				for await (const [completion, abort] of processOpenAIAPIChatCompletionsStream(stream)) {
					...
					switch (choice.finish_reason.unwrap()) {
						case 'error': {
							yield* AsyncResult.Err(new Error('Error in chat completion', { cause: completion }));
							break;
						}
						case 'stop': {
							abort();
						...
						case 'tool_calls': {
							...
							onChunk.inspect((onChunk) =>
								onChunk((newChunks.at(-1) as TMessage & { type: 'llm' }).chunks)
							);
