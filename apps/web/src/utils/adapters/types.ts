...
const adapterSchema = type({
	id: 'string',
	makeFetcher: type('Function').as<(baseUrl: Option<string>, token: Option<string>) => $Fetch>(),
	fetchAllModels:
		type('Function').as<(fetcher: $Fetch, opts: FetchOptions) => AsyncResult<TModel[], Error>>(),
	handleChatCompletion:
		...
		>(),
	processContentDelta: type('Function').as<<T extends [any]>(...args: T) => Option<string>>(),
	processReasoningDelta: type('Function').as<<T extends [any]>(...args: T) => Option<string>>(),
