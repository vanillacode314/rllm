...
function createForm<
  TSchema extends type.Any<object>,
  const TInitial = NoInfer<TSchema['infer']>,
>(schema: TSchema, initial: () => TInitial) {
...
function parseFormErrors<T extends type.errors>(
  ...
  return input.reduce(
    (errors, error) => {
      ...
      errors[name] ??= []
      if (error.code === 'union') {
        const subErrors = parseFormErrors(
          (error as ArkError<'union'>).errors as type.errors,
        )
