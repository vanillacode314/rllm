...
type TOptimizationState = {
  ...
  alraedyUpdatedChatIds: Set<string>
...
async function produceMessagesOptimizationState(
  ...
  const state = {
    ...
    alraedyUpdatedChatIds: new Set<string>(),
    ...
    switch (message.user_intent) {
      ...
      case 'update_chat': {
        ...
        if (
          ...
          state.alraedyUpdatedChatIds.has(chatId as never)
        ) {
          state.messagesToDelete.add(message.timestamp)
          break
        }
        state.alraedyUpdatedChatIds.add(chatId)
        break
      }
...
async function optimizeMessages<T extends TMessage>(
  ...
  return messages
    .values()
    .filter((message) => !state.messagesToDelete.has(message.timestamp))
    .toArray()
}
