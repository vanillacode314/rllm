...
async function optimizeMessages<T extends TMessage>(messages: T[]): Promise<T[]> {
...
async function optimizeStorage() {
	const messages = await db.select().from(schema.messages).orderBy(asc(schema.messages.timestamp));
	...
	await transaction(async (tx) => {
		await tx.query(
			db
				...
				.where(inArray(schema.messages.timestamp, state.messagesToDelete.values().toArray()))
		);
		await tx.query(
			db
				.delete(schema.chats)
				.where(inArray(schema.chats.id, state.deletedChatIds.values().toArray()))
		);
		await tx.query(
			db
				.delete(schema.providers)
				.where(inArray(schema.providers.id, state.deletedProviderIds.values().toArray()))
		);
		await tx.query(
			db.delete(schema.mcps).where(inArray(schema.mcps.id, state.deletedMCPIds.values().toArray()))
		);
	});
	await db.run(sql`VACUUM;`);
}
async function produceMessagesOptimizationState(messages: TMessage[]): Promise<TOptimizationState> {
	const state = {
