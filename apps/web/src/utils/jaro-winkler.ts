 ...
 */
export type CharArrayResolvable = CharArray | string
...
export function jaroWinkler(
  stringCompare: CharArrayResolvable,
  stringCompareWith: CharArrayResolvable,
) {
  ...
  const similarity =
    (matches / l1 + matches / l2 + (matches - transpositions) / matches) / 3
