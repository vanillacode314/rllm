...
class ReactiveTreeNode<const T> extends TreeNode<T> {
	...
	static fromTree<T>(tree: TTreeNode<T>): ReactiveTreeNode<T> {
		const reactiveTree = new this<T>();
		reactiveTree.setValue(tree.value);
		const children = tree.children.map((child) => this.fromTree(child));
		for (const child of children) {
			child.setParent(reactiveTree);
		}
		reactiveTree.setChildren(children);
		return reactiveTree;
	}
	removeParent() {
	...
	toJSON(): JsonTreeNode<T> {
		return {
			value: unwrap(this.value.toNull()),
			children: this.children.map((child) => child.toJSON())
		};
	}
}
