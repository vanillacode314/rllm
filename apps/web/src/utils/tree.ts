...
interface TTreeNode<T> {
  ...
  iter(
    path: number[],
  ): IteratorObject<{ node: Option<TTreeNode<T>>; path: number[] }>
  get value(): Option<T>
}
...
class ReactiveTreeNode<const T> extends TreeNode<T> {
  ...
  protected _parentSignal: Signal<Option<TTreeNode<T>>> = createSignal(
    Option.None(),
  )
  protected _valueSignal: Signal<Option<T>> = createSignal(Option.None())
