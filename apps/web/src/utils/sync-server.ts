...
import { ValidEvent } from '~/queries/mutations';
...
const messagesResponseSchema = Type.Object({
	nextAfter: Type.Union([Type.String(), Type.Null()]),
...
export async function parseEventsFromServer(data: any, aesKey: CryptoKey) {
	const events = [] as Array<Type.Static<typeof ValidEvent> & { syncedAt: string }>;
