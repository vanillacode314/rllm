...
type TUpdate<
  T extends SQLiteTableWithColumns<any> = SQLiteTableWithColumns<any>,
> = {
...
async function applyUpdate(
  ...
  const columns = Object.keys(update.data).filter(
    (column) => update.data[column] !== undefined,
  )
    ...
    case 'update': {
      for (const column of columns) {
        await tx.query({
          ...
          params: [update.data[column], id, timestamp].map((value) =>
            toSql(value),
          ),
        })
        await tx.query({
...
async function receiveMessages(
  ...
  for (const message of messages) {
    try {
      ...
      const updates = await processMessage(validMessage.assert(message), { tx })
