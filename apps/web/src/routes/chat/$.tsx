...
import { batch, createComputed, createMemo, createSignal, onMount, Show, untrack } from 'solid-js';
	...
	loader: async ({ deps, params }) => {
		...
		let [selectedProvider, providers] = await Promise.all([
			queryClient
				...
				.then(async (id) => {
					return id ? queryClient.ensureQueryData(queries.providers.byId(id)) : null;
				}),
...
function ChatPageComponent() {
	...
	const [chat, setChat] = createWritableMemo<TChat>(() => {
		...
		return {
			...chatQuery.data,
			messages: ReactiveTree.fromJSON<TMessage>(unwrap(chatQuery.data.messages))
		};
	});
	...
	function finalizeChat(chat: TChat, path: number[], error?: string) {
		const oldNode = chat.messages.traverse(path).expect('should be able to traverse to node');
		const node = chat.messages
			.traverse([...path, oldNode.children.length - 1])
			.expect('should be able to traverse to node');
	...
	const sendPrompt = useMutation(() => ({
		...
		async onSuccess(_, { path, model, provider, chat }, context) {
			finalizeChat(chat, path);
			...
			await logger.dispatch({ type: 'updateChat', data });
		}
	}));
