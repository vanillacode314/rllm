  ...
  queries: {
    all: () => {
      return Object.assign(
        queryOptions({
          ...
          queryFn: () =>
            providers.fetchers.getAllProviders().orderBy(schema.providers.name),
          staleTime: Infinity,
        }),
        {
          _ctx: {
    ...
    selected: () =>
      queryOptions({
        ...
        queryFn: async () => {
          const selectedProviderId = await userMetadata.fetchers.byId(
      ...
      queryOptions({
        ...
        queryFn: () => {
          ...
          return db
            ...
            .where(
              and(
                eq(schema.providers.deleted, false),
      ...
      queryOptions({
        ...
        queryFn: async () => {
          const selectedModelId =
            await userMetadata.fetchers.byId('selected-model-id')
  ...
  queries: {
    all: () =>
      queryOptions({
        queryKey: ['db', 'chats'],
        queryFn: () =>
          db.select().from(schema.chats).where(eq(schema.chats.deleted, false)),
        staleTime: Infinity,
      }),
    byId: (id: string) =>
      queryOptions({
        ...
        queryFn: () =>
          db
            ...
            .from(schema.chats)
            .where(
              and(eq(schema.chats.deleted, false), eq(schema.chats.id, id)),
            )
            .then((rows) => rows[0] ?? null),
        staleTime: Infinity,
      }),
...
const mcps = {
  fetchers: {
    getAllMcps: () =>
      db.select().from(schema.mcps).where(eq(schema.mcps.deleted, false)),
  },
  queries: {
    all: () =>
      Object.assign(
        queryOptions({
          queryKey: ['db', 'mcps'],
          queryFn: () =>
            mcps.fetchers.getAllMcps().orderBy(desc(schema.mcps.createdAt)),
          staleTime: Infinity,
        }),
        {
          _ctx: {
            clients: (proxy?: string | undefined) =>
              queryOptions({
                ...
                queryFn: async () => {
                  const $mcps = await mcps.fetchers
                    .getAllMcps()
                    .orderBy(schema.mcps.name)
                  return $mcps.map(
                    (mcp) =>
                      new MCPClient(
                        mcp.name,
                        proxy ? proxy.replace('%s', mcp.url) : mcp.url,
                      ),
                  )
                },
                staleTime: Infinity,
