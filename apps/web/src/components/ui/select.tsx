...
type SelectListboxProps<T extends ValidComponent = 'ul'> =
  SelectPrimitive.SelectListboxProps<T> & {
    class?: string | undefined
  }
const SelectListbox = <T extends ValidComponent = 'ul'>(
  props: PolymorphicProps<T, SelectListboxProps<T>>,
) => {
  const [local, others] = splitProps(props as SelectListboxProps, [
    'class',
    'ref',
  ])
  return (
    <SelectPrimitive.Listbox
      class={cn('m-0 p-1', local.class)}
      {...others}
      ref={local.ref}
    />
  )
