...
export function TheCommandPrompt() {
	...
	createShortcut(['Control', 'k'], () => setOpen((open) => !open));
	createShortcut(['Meta', 'k'], () => setOpen((open) => !open));

	const navigate = useNavigate();
	const location = useLocation();
	const matchRoute = useMatchRoute();
	const isNewChatRoute = matchRoute({
		to: '/chat/$',
		params: { _splat: 'new' },
		search: { id: { undefined } },
		includeSearch: true
	...
	const fetchers = createMemo(() =>
		(providers.isSuccess ? providers.data : []).map((provider) => ({
			fetcher: openAiAdapter.makeFetcher(
				Option.fromNull(proxifyUrl(provider.baseUrl)),
				Option.fromNull(provider.token)
			),
			provider
		}))
	);
			...
			case 'default':
				return {
					Actions: [
						...
						{
							label:
								currentChat.isSuccess ? `Delete Chat (${currentChat.data.title})` : 'Delete Chat',
						...
						{
							label:
								currentChat.isSuccess ? `Rename Chat (${currentChat.data.title})` : 'Rename Chat',
	...
	async function deleteChat(id: string) {
		...
		await Promise.all([
			queryClient.invalidateQueries(queries.chats.all()),
			queryClient.invalidateQueries(queries.chats.byId(id))
		]);
	}
