...
export function Chat(props: Props): JSXElement {
  ...
  const [{ canScroll, autoScroll, shouldAutoScroll }, { scrollToBottom }] = useAutoScroll({
    enabled: () => false
  });
...
function LLMToolCallChunk(props: {
  ...
  const contentIsJson = createMemo(() => {
    if (!props.chunk.success) return false;
    try {
      ...
      <CollapsibleContent class="space-y-2">
        <article class="space-y-0.5">
          ...
          <div
            ...
            innerHTML={requestHtml.isSuccess ? requestHtml.data : 'Loading...'}
          />
        </article>
        <article class="space-y-0.5">
          ...
          <Show
            fallback={
              <div class="rounded border p-3 text-xs max-h-96 overflow-auto whitespace-pre-wrap font-mono">
                <span class="icon-[svg-spinners--180-ring-with-bg]" />
              </div>
            }
          ...
          >
            <Show
              fallback={
                <div
                  ...
                  classList={{ 'text-red-600 dark:text-red-400': !props.chunk.success }}
            ...
            >
              <div
                class="rounded border p-3 text-xs overflow-auto whitespace-pre font-mono"
                classList={{ 'text-red-600 dark:text-red-400': !props.chunk.success }}
                innerHTML={responseHtml.isSuccess ? responseHtml.data : 'Loading...'}
              />
            </Show>
          </Show>
        </article>
