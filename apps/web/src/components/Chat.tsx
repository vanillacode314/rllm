...
export function Chat(props: Props): JSXElement {
	...
	const scrollToBottom = debounce(
		() => {
			...
			$ref.addEventListener('scrollend', () => local.setScrollingStatus('none'), {
				once: true,
				passive: true
			});
			$ref.scrollTo({
	...
	createRenderEffect(() => {
		...
		if (!height || untrack(() => local.scrollingStatus === 'manual')) return;
		scrollToBottom();
	});
	...
	return (
		<div class="relative overflow-hidden grid">
			<div
				class={cn('touch-none overflow-auto', props.class)}
				ref={(el) => {
					setRef(el);
			...
			>
				<Button
					class="absolute bottom-36 left-1/2 rounded-full size-8 text-secondary-foreground/50 hover:text-secondary-foreground bg-secondary/50 hover:bg-secondary/100 border border-secondary-foreground/25 motion-preset-fade motion-duration-300 transition-colors backdrop-blur-xs will-change-transform"
					onClick={() => {
...
function LLMChat(props: {
	...
	return (
		<Card class="bg-transparent border-none shadow-none">
			<CardHeader class="p-2 grid gap-4 pb-6 grid-cols-[1fr_auto] items-center overflow-hidden">
...
function UserChat(props: {
	...
	return (
		<>
			<Card class="border-ring border-3">
				<div class="flex empty:pb-0 p-4 gap-2 justify-end items-center relative">
					<div class="absolute left-4 text-sm font-bold top-4">
