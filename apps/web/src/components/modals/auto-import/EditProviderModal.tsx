...
const [providerIdToEdit, setProviderIdToEdit] = createSignal<string | false>(
  false,
)
...
export function EditProviderModal() {
  ...
  const [
    { form, formErrors },
    { setFormErrors, setForm, resetForm, resetFormErrors },
        ...
        <form
          ...
          onSubmit={async (event) => {
            ...
            await queryClient.invalidateQueries(
              queries.providers.byId(parsedFormData.id),
            )
            setProviderIdToEdit(false)
          }}
        >
          ...
          <TextField class="grid grid-cols-4 items-center gap-x-4 gap-y-1.5">
                ...
                {(id, index) => (
                  <Badge>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newModels = form.defaultModelIds.filter(
                          (_, i) => i !== index(),
                        )
            ...
            <ValidationErrors
              class="col-start-2 col-end-5"
              errors={formErrors.defaultModelIds}
            />
          </TextField>
          <DialogFooter>
            <Button type="submit">
